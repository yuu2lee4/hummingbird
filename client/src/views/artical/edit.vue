...
export default class articalEdit extends Vue {
    files = []
    post = {
        title: '',
        category: '',
    ...
    async beforeRouteEnter(to, from, next) {
        if (!window.$store.state.account) {
            Vue.toast('登录之后才能编辑文章！', { className: 'et-warn' });
            return next({ name: 'home' });
        }
        const getPost = $api.getPost$(to.params.id);
        const postCategoryList = $api.postCategoryList();
        const [post, categoryList] = await Promise.all([getPost, postCategoryList]);
        next((vm) => {
    ...
    inputFile(newFile, oldFile) {
        ...
        if (newFile) {
            // 上传成功的文件
            if (newFile.success && newFile.response && newFile.response[0]) {
                this.$nextTick(() => {
                    if (clipboard) clipboard.destroy();
            ...
            // 上传错误
            if (oldFile && newFile.error !== oldFile.error) {
