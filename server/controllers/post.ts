...
export const save = async ctx=>{
    ...
    if(id){
        ...
        const post = await Post.findOne({ title, author }).exec();
        if(post){
...
export const list = async ctx=>{
    ...
    if(ids && ids.length){
        post = await Post.find({_id: {$in: ids} }).select('title meta').exec();
    }else{
        post = await Post.find().select('title meta').exec();
    }
    if(post){
        ctx.body = {code:0,data: post};
    }
...
function baseSearch (pageSize = 10, page = 1, condition = {}) {
    ...
    const searchTagPromise = Post.find(condition).sort('-meta.updateAt').select('title meta').limit(pageSize).skip(skipNum);
    const countNumerPromise = Post.count(condition);
